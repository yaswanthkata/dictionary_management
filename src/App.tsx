...
import { saveToLocalStorage, validateRows, getItemsFromLocalStorage } from './services';
...
class App extends React.Component<IProps, IAppState> {
  ...
  componentDidMount() {
    const dictionaries = getItemsFromLocalStorage();
  ...
  handleValidation = (rows: DictionaryRow[]) => {
    const newItems = [...this.state.dictionaries];
    const selectedItem = newItems.find(d => d.id === this.state.selectedDictionary.id);
    if (selectedItem) {
      selectedItem.rows = [...validateRows(rows)];
      this.setState({ dictionaries: newItems, dictionaryRows: [...rows] }, () => this.save());
    }
  }
  render() {
